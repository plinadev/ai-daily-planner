...
export default function MessageItem({
  ...
  return (
    <div className="space-y-1">
      <div
        className={`text-xs font-semibold mb-1 ${isUser ? "text-right" : "text-left"}`}
      ...
      {imageParts.length > 0 && (
        <div
          className={`flex flex-wrap gap-2 ${isUser ? "justify-end" : "justify-start"} mb-1`}
        >
      ...
      <div className={`chat ${isUser ? "chat-end" : "chat-start"}`}>
        {(!isUser || text) && (
          <div
            className={`chat-bubble text-[15px] leading-relaxed ${isUser ? "" : "bg-transparent pl-0"}`}
            style={
              isUser
                ? { background: "#9fa0c322", color: "#1a1a2e" }
                : { color: "#1a1a2e" }
            }
          >
            {isUser ? (
